...
class XiaomiRoborockVacuum {
  ...
  initialiseServices() {
    ...
    const serviceNames = this.config.serviceNames || {};
    ...
    this.services.fan = new Service.Fan(
      serviceNames.speed || this.config.name,
      "Speed"
    );
    ...
    if (this.config.waterBox) {
      this.services.waterBox = new Service.Fan(
        serviceNames.waterBox || `${this.config.name} Water Box`,
        "Water Box"
      );
    ...
    if (this.config.dustCollection) {
      this.services.dustCollection = new Service.Fan(
        serviceNames.dustCollection || `${this.config.name} Dust Collection`,
        "Dust Collection"
      );
    ...
    if (this.config.findMe) {
      this.services.findMe = new Service.Switch(
        serviceNames.findMe || `${this.config.name} ${this.config.findMeWord}`,
        "FindMe Switch"
      );
    ...
    if (this.config.dock) {
      this.services.dock = new Service.OccupancySensor(
        serviceNames.dock || `${this.config.name} Dock`
      );
      this.services.dock
