...
module.exports = class extends Vacuum.with(
  ...
  loadProperties(props) {
    // We override loadProperties to use get_status. Consumables are optional: newer or regional
    // Roborock firmwares can fail this auxiliary call, but status should still refresh.
    props = props.map((key) => this._reversePropertyDefinitions[key] || key);

    return this.call("get_status").then((statusResult) => {
      const status = statusResult[0] || {};
      return this.call("get_consumable")
        .catch(() => [{}])
        .then((consumableResult) => {
          const consumables = consumableResult[0] || {};
          const mapped = {};
          props.forEach((prop) => {
